...
class Trans_high(nn.Module):
    ...
    def forward(self, x, pyr_original, fake_low):
        ...
        res0 = self.trans_gate_block_0(pyr_original[-2], fake_low_up * mask0)
        ...
        for i in range(1, self.num_high):
            ...
            res = self.trans_gate_block(pyr_original[-2 - i], current_img_up * mask)
...
class FPMTNet(nn.Module):
    def __init__(self, img_dim, parse_dim, num_high=3):
        super(FPMTNet, self).__init__()

        self.num_high = num_high
        self.lap_pyramid = networks.Lap_Pyramid_Conv(num_high)

        self.trans_low = Trans_low(img_dim, parse_dim)
        self.trans_high = Trans_high(num_residual_blocks=2, pyr_recon_f=self.lap_pyramid.pyramid_recons,
                                     num_high=num_high)
